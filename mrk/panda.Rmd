...
Now we check for every genome except the reference genome (that provided all the systematic genes) how many systematic genes we have in the pangenome compared to what we had in the annotations. This is particularly relevant for S288C which is basically our version of SGD.

The script reads the pangenome sub-block table "sts-by-haplos.txt" and extracts all haplotype identifiers, excluding the reference genome and metadata columns such as Class_id, Features_id, Ν_pres, F_pres, N_feats, N_feats_sys, and N_feats_rid. For each remaining ID column, it identifies the rows in which the ID is present and at least one systematic gene is annotated (N_feats_sys > 0), and uses these rows to compute the number of systematic genes reported in the pangenome table. The script then retrieves the corresponding nuclear GFF annotation file for that genome, extracts all entries of type "gene", and counts the systematic gene names contained in their attribute field. For each identifier, the script compares the sets of systematic gene names detected in the pangenome and in the GFF annotation, determines whether any genes were lost or gained when the pangenome was calculated, and finally writes a summary table listing, for every ID, the number of systematic genes detected in the pangenome, the number detected in the GFF annotation, and the counts of lost and gained genes. For each identifier a file listing the lost and gained genes is also produced.

Entries bearing multiple gene labels (e.g. "YAL068C/YBR301W" in the annotation files or "YAL068C,YBR301W" in the pangenome table) are counted only once. This behaviour can be modified changing `length()` with `sum()`.

```{r checks the systematic genes}
## header ---------------------------------------------------------------------
